...
mutable struct MultUpdate{T} 
    ...
    function MultUpdate{T}(;obj::Symbol=:mse,
                         ...
                         verbose::Bool=false,
                         tol::Real=cbrt(eps(T)),
                         lambda::Real=sqrt(eps(T))) where T <: AbstractFloat
...
struct MultUpdMSE_State{T}
    ...
    function MultUpdMSE_State{T}(X, W::Matrix{T}, H::Matrix{T}) where T <: AbstractFloat
...
struct MultUpdDiv_State{T}
    ...
    function MultUpdDiv_State{T}(X, W::Matrix{T}, H::Matrix{T}) where T<:AbstractFloat
