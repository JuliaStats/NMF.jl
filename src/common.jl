...
struct Result{T}
    ...
    function Result(W::Matrix{T}, H::Matrix{T}, niters::Int, converged::Bool, objv::Any) where T <: AbstractFloat
...
function nmf_skeleton!(updater::NMFUpdater{T},
                       ...
                       maxiter::Int, verbose::Bool, tol) where T <: AbstractFloat
    objv = convert(T, NaN)
    ...
    ret=Result(W, H, t, converged, objv)
    return ret
end
